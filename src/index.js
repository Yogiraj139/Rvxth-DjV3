...
const tokenEnv = getEnvValue(['DISCORD_TOKEN', 'BOT_TOKEN', 'TOKEN']);
const discordToken = normalizeBotToken(tokenEnv.value);
...
async function playNext(guildId) {
  ...
  const downloader = spawn(
    ...
    song.url,
      ...
      '-',
      '--format',
      'bestaudio[ext=webm][acodec=opus]/251/250/249',
      '--quiet',
      '--no-warnings',
      '--no-playlist'
    ],
  ...
  const resource = createAudioResource(downloader.stdout, {
